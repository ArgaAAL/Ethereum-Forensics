...
df = pd.read_csv(r'D:\AAL\Coding\piton\BlockchainAnalyzer\xgboost\ethereum_features_output.csv')
...
print("   - Clipping infinite values to max float32 value for stability.")
...
with open('eth_scaler_parameters.json', 'w') as f:
...
try:
    ...
    onnx_filename = 'eth_ransomware_model_mlp.onnx'
...
with open('eth_model_metadata.json', 'w') as f:
...
if not illicit_indices.empty:
    ...
    with open('eth_test_sample.json', 'w') as f:
        json.dump(test_sample_dict, f, indent=4)
    print("   - ✅ Test sample for Rust canister saved to 'eth_test_sample.json'")
else:
    print("   - ⚠️  No illicit samples found to create a test case.")

# Save the Python-specific model and scaler for other uses
joblib.dump({'model': mlp_model, 'scaler': scaler}, 'eth_python_mlp_model_and_scaler.joblib')
print("   - ✅ Python model and scaler saved to 'eth_python_mlp_model_and_scaler.joblib'")
...
print(f"  1. Clean ONNX Model:      eth_ransomware_model_mlp.onnx")
print(f"  2. Scaler Parameters:     eth_scaler_parameters.json")
print(f"  3. Deployment Metadata:   eth_model_metadata.json")
print(f"  4. Canister Test Sample:  eth_test_sample.json (if created)")
print("\n🚨 DEPLOYMENT WORKFLOW:")
